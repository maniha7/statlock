    ...
    return(
        <div ref={itemRef} onMouseEnter={()=>openPopup()} onMouseLeave={()=>props.unhover()} className="flex select-none my-[6px] mx-[6px]" onClick={()=>{console.log(item)}}>
            <div className={`flex flex-col items-center items-center max-w-[75px] hover:opacity-80 hover:cursor-pointer ${bgColor}`} style={{borderRadius:8}}>
                ...
                {/* Item Label*/}
                <div className="px-[2px] pt-2 pb-1 flex flex-1 justify-center" style={{backgroundColor:"rgba(255,255,255,.7)", position:'relative', borderBottomRightRadius:8, borderBottomLeftRadius:8, width:'100%',}}>
                    {/* Active Tag */}
                    {item.activation=="instant_cast" &&
                        <div ref={tagRef} className="py-[.5px] px-[8px]" style={{backgroundColor:gColors.itemBlack, borderRadius:3, position:'absolute', top:-tagHeight}}>
